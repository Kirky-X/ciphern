...
fn test_scheduled_key_rotation() {
    ...
    lifecycle_manager.set_policy(Algorithm::AES256GCM, policy).unwrap();
...
fn test_rotation_with_data_migration() {
    ...
    // 生成密钥并加密数据
    let old_key_id = key_manager.generate_key_with_lifecycle(Algorithm::AES256GCM, &lifecycle_manager).unwrap();
    ...
    // 执行轮换
    let new_key_id = key_manager.rotate_key(&old_key_id, Algorithm::AES256GCM, &lifecycle_manager).unwrap();
...
fn test_rotation_policy_enforcement() {
    ...
    let time_test_key_id = key_manager.generate_key_with_lifecycle(Algorithm::AES256GCM, &lifecycle_manager).unwrap();
    
    // 等待时间过期 - 密钥生命周期为1秒，等待1.5秒确保过期
    thread::sleep(Duration::from_millis(1500));
    
    // 时间过期应该触发轮换警告
    let time_warning = lifecycle_manager.get_rotation_warning(&time_test_key_id).unwrap();
...
fn test_emergency_key_rotation() {
    ...
    lifecycle_manager.set_policy(Algorithm::AES256GCM, emergency_policy).unwrap();
