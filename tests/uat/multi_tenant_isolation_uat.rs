...
fn test_tenant_data_encryption_isolation() {
    ...
    // 验证租户无法解密其他租户的数据
    assert!(cipher
        .decrypt(&tenant1_key, &tenant1_encrypted, None)
        .is_ok()); // 租户1可以解密自己的数据
    assert!(cipher
        .decrypt(&tenant2_key, &tenant2_encrypted, None)
        .is_ok()); // 租户2可以解密自己的数据
    ...
    let handles = vec![
        {
            ...
            thread::spawn(move || {
                for i in 0..10 {
                    let _key_id = manager.generate_key(Algorithm::AES256GCM).unwrap();
...
fn test_tenant_key_rotation_isolation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 验证轮换警告
    let warning1 =
        lifecycle_manager1.get_rotation_warning_for_key(tenant1_manager.as_ref(), &key1)?;
    let warning2 =
        lifecycle_manager2.get_rotation_warning_for_key(tenant2_manager.as_ref(), &key2)?;
...
fn test_tenant_deletion_cascade() {
    ...
    println!(
        "租户密钥数量: {}",
        tenant_manager.list_keys().unwrap().len()
    );
}
...
fn test_cross_tenant_attack_prevention() {
    ...
    let _encrypted = cipher.encrypt(&tenant1_key, data, None).unwrap();
