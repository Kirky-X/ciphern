...
mod tests {
    use ciphern::fips::{
        FipsAlgorithmValidator, FipsContext, FipsError, FipsMode, FipsSelfTestEngine,
    };
    ...
    fn test_fips_algorithm_approval() {
        ...
        for alg in approved {
            assert!(
                FipsAlgorithmValidator::is_algorithm_approved(&alg),
                "{:?} should be approved",
                alg
            );
        }

        let unapproved = vec![Algorithm::SM4GCM];
