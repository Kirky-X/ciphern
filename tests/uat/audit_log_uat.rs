...
use ciphern::audit::{AuditLogger, AuditLog};
...
fn test_unauthorized_access_logging() {
    ...
    assert_eq!(unauthorized_log.tenant_id.as_deref(), Some("tenant2"), "日志应该包含尝试访问的租户ID");
    assert!(unauthorized_log.details.contains("tenant1"), "日志应该包含被访问的租户ID");
    assert_eq!(unauthorized_log.operation, "KEY_ACCESS", "日志应该包含操作类型");
    assert_eq!(unauthorized_log.access_type, "unauthorized", "日志应该标记为非授权访问");
}
...
fn test_authorized_access_logging() {
    ...
    // 查找授权访问记录
    let authorized_logs: Vec<_> = logs.iter()
    ...
    assert_eq!(authorized_log.tenant_id.as_deref(), Some("tenant1"), "日志应该包含租户ID");
    assert_eq!(authorized_log.access_type, "authorized", "日志应该标记为授权访问");
}
...
fn test_key_operations_audit_logging() {
    ...
    // 5. 设置最大使用次数
    tenant_manager.set_key_max_usage(&key_id, Some(100)).unwrap();
    std::thread::sleep(Duration::from_millis(200));
    ...
    // 验证所有记录都包含租户信息
    for log in &logs {
        if vec!["KEY_GENERATE", "KEY_LIST", "KEY_ACTIVATE", "KEY_SUSPEND", "KEY_MAX_USAGE_SET", "KEY_DESTROY"].contains(&log.operation.as_str()) {
            assert_eq!(log.tenant_id.as_deref(), Some("audit_test_tenant"), "记录 {} 应该包含租户ID", log.operation);
            assert_eq!(log.access_type, "authorized", "授权操作应该标记为authorized");
        }
    }
}
...
fn test_tenant_id_extraction_from_key() {
    ...
    let unauthorized_log = &unauthorized_logs[0];
    assert!(unauthorized_log.details.contains("Tenant tenant2 attempted to access key from tenant tenant1"), 
            "日志应该包含详细的非授权访问信息");
}
...
fn test_audit_log_security_alerts() {
    ...
    assert!(unauthorized_logs.len() >= 20, "应该至少有20条非授权访问记录 (10次 * 2个密钥)");
    
    // 验证所有记录都包含安全警告
    for log in &unauthorized_logs {
        assert!(log.details.contains("SECURITY ALERT"), "每条非授权访问记录都应该包含安全警告");
        assert_eq!(log.tenant_id.as_deref(), Some("tenant2"), "记录应该包含攻击者租户ID");
