...
impl ApiClient {
    ...
    /// 密钥协商过程
    fn negotiate_keys(&self, server_public_key: &[u8]) -> Result<(String, String), ciphern::error::CryptoError> {
        // FIPS 140-3 合规性检查
        ...
        let shared_secret = ciphern::key::derivation::Hkdf::derive(&master_key, server_public_key, b"api-key-negotiation", Algorithm::AES256GCM)?;
    ...
    /// 派生并创建密钥
    fn derive_and_create_key(&self, shared_secret: &[u8], context: &[u8], algorithm: Algorithm) -> Result<String, ciphern::error::CryptoError> {
        // 使用HKDF派生密钥材料
        use ring::hkdf;
        ...
        let okm = prk.expand(&info, ring::hkdf::HKDF_SHA256)
            .map_err(|_| ciphern::error::CryptoError::EncryptionFailed("HKDF Expand failed".into()))?;
        
        let mut derived_bytes = vec![0u8; algorithm.key_size()];
        okm.fill(&mut derived_bytes)
            .map_err(|_| ciphern::error::CryptoError::EncryptionFailed("HKDF Fill failed".into()))?;
    ...
    fn validate_fips_compliance(&self) -> Result<(), ciphern::error::CryptoError> {
        ...
        if !self.is_fips_mode_enabled() {
            return Err(ciphern::error::CryptoError::InvalidParameter("FIPS mode required".into()));
        }
        
        // 2. 验证使用的算法是否在FIPS批准列表中
        let approved_algorithms = vec![Algorithm::ECDSAP384, Algorithm::AES256GCM, Algorithm::SHA256];
        for algorithm in approved_algorithms {
            if !FipsAlgorithmValidator::is_algorithm_approved(&algorithm) {
                return Err(ciphern::error::CryptoError::UnsupportedAlgorithm(format!("{:?}", algorithm)));
            }
        }
    ...
    fn validate_key_management_compliance(&self) -> Result<(), ciphern::error::CryptoError> {
        ...
        if key.state() != KeyState::Active {
            return Err(ciphern::error::CryptoError::KeyError("Invalid key state".into()));
        }
    ...
    fn validate_key_strength(&self, key: &Key) -> Result<(), ciphern::error::CryptoError> {
        ...
        match key.algorithm() {
            Algorithm::AES128GCM => {
                if key_len != 16 {
                    return Err(ciphern::error::CryptoError::KeyError("Invalid AES-128 key length".into()));
                }
            },
            Algorithm::AES192GCM => {
                if key_len != 24 {
                    return Err(ciphern::error::CryptoError::KeyError("Invalid AES-192 key length".into()));
                }
            },
            Algorithm::AES256GCM => {
                if key_len != 32 {
                    return Err(ciphern::error::CryptoError::KeyError("Invalid AES-256 key length".into()));
                }
            },
            Algorithm::ECDSAP384 => {
                if key_len < 48 {
                    return Err(ciphern::error::CryptoError::KeyError("Invalid ECDSA P-384 key length".into()));
                }
            },
            _ => return Err(ciphern::error::CryptoError::UnsupportedAlgorithm(format!("{:?}", key.algorithm()))),
        }
    ...
    /// 发送加密消息
    fn send_encrypted_message(&self, enc_key_id: &str, mac_key_id: &str, data: &[u8]) -> Result<Vec<u8>, ciphern::error::CryptoError> {
        // 获取密钥
    ...
    /// 接收并验证加密消息
    fn receive_encrypted_message(&self, enc_key_id: &str, mac_key_id: &str, message: &[u8]) -> Result<Vec<u8>, ciphern::error::CryptoError> {
        if message.len() < 32 {
            return Err(ciphern::error::CryptoError::InvalidParameter("Invalid message length".into()));
        }
        ...
        if calculated_mac != mac {
            return Err(ciphern::error::CryptoError::InvalidParameter("Authentication failed".into()));
        }
...
impl ApiServer {
    ...
    /// 处理客户端密钥协商请求
    fn handle_key_negotiation(&self, client_public_key: &[u8]) -> Result<(String, String), ciphern::error::CryptoError> {
        // 1. 生成服务器临时密钥对
        ...
        // 2. 使用HKDF派生共享密钥
        let shared_secret = ciphern::key::derivation::Hkdf::derive(&server_key, client_public_key, b"api-key-negotiation", Algorithm::AES256GCM)?;
        
        // 3. 生成加密密钥和MAC密钥
        let encryption_key = ciphern::key::derivation::Hkdf::derive(&shared_secret, b"", b"encryption-key", Algorithm::AES256GCM)?;
        let mac_key = ciphern::key::derivation::Hkdf::derive(&shared_secret, b"", b"mac-key", Algorithm::AES256GCM)?;
    ...
    /// 处理加密消息
    fn handle_encrypted_message(&self, enc_key_id: &str, mac_key_id: &str, message: &[u8]) -> Result<Vec<u8>, ciphern::error::CryptoError> {
        if message.len() < 32 {
            return Err(ciphern::error::CryptoError::InvalidParameter("Invalid message length".into()));
        }
        ...
        // 3. 处理请求（这里只是回显）
        let response = format!("Server response to: {}", String::from_utf8_lossy(&plaintext));
...
mod tests {
    ...
    fn test_encrypted_message_exchange() {
        ...
        let client = ApiClient::new_with_key_manager("test-client".to_string(), shared_key_manager.clone());
        let server = ApiServer::new_with_key_manager("test-server".to_string(), shared_key_manager.clone());
        ...
        let encrypted_message = client.send_encrypted_message(&enc_key_id, &mac_key_id, message).unwrap();
        
        // 服务器处理消息
        let response = server.handle_encrypted_message(&enc_key_id, &mac_key_id, &encrypted_message).unwrap();
        
        // 客户端接收响应
        let decrypted_response = client.receive_encrypted_message(&enc_key_id, &mac_key_id, &response).unwrap();
        
        assert!(String::from_utf8_lossy(&decrypted_response).contains("Server response to: Hello, secure world!"));
    }
    ...
    fn test_key_derivation_security() {
        ...
        for i in 0..key_pairs.len() {
            for j in i+1..key_pairs.len() {
                assert_ne!(key_pairs[i].0, key_pairs[j].0, "Encryption keys should be unique");
