...
fn uat_database_encryption() {
    ...
    // 2. Encrypt
    let encrypted_id_card = cipher
        .encrypt(&key_manager, &key_id, user_data.id_card.as_bytes())
        .unwrap();
    let encrypted_phone = cipher
        .encrypt(&key_manager, &key_id, user_data.phone.as_bytes())
        .unwrap();
    ...
    let decrypted_id_card = String::from_utf8(
        cipher
            .decrypt(&key_manager, &key_id, &db_record.encrypted_id_card)
            .unwrap(),
    )
    .unwrap();
    ...
    let decrypted_phone = String::from_utf8(
        cipher
            .decrypt(&key_manager, &key_id, &db_record.encrypted_phone)
            .unwrap(),
    )
    .unwrap();
