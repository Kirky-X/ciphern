...
fn test_key_rotation_workflow() {
    ...
    let cipher = ciphern::provider::registry::REGISTRY
        .get_cipher(Algorithm::AES256GCM)
        .unwrap();
    let ciphertext = cipher
        .encrypt(&key_manager, &old_key_id, plaintext)
        .unwrap();
    ...
    // 4. 旧密钥仍可解密
    let decrypted_old = cipher
        .decrypt(&key_manager, &old_key_id, &ciphertext)
        .unwrap();
    ...
    let new_ciphertext = cipher
        .encrypt(&key_manager, &new_key_id, new_plaintext)
        .unwrap();

    let decrypted_new = cipher
        .decrypt(&key_manager, &new_key_id, &new_ciphertext)
        .unwrap();
    assert_eq!(new_plaintext, &decrypted_new[..]);

    // 6. 验证隔离性
    assert!(cipher
        .decrypt(&key_manager, &new_key_id, &ciphertext)
        .is_err());
}
...
fn test_usage_based_rotation() {
    ...
    for i in 0..3 {
        ...
        let _ciphertext = cipher
            .encrypt(&key_manager, &key_id, data.as_bytes())
            .unwrap();
    }
...
fn test_key_rotation_with_alias() {
    ...
    let key_id = key_manager
        .generate_key_with_alias(Algorithm::AES256GCM, alias)
        .unwrap();
