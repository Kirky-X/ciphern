...
use ciphern::key::derivation::Hkdf;
...
fn test_key_derivation_different_algorithms() {
    ...
    // 派生不同算法的密钥 - 使用主密钥的secret bytes作为盐值（与UAT测试相同）
    let salt = master_key.secret_bytes().unwrap().as_bytes(); // 使用主密钥的secret作为盐值
    
    // 测试所有对称加密算法的密钥派生
    // 现在支持所有密钥长度：16字节、24字节、32字节
    
    // AES-256 (32字节)
    let derived_aes256 = Hkdf::derive(&master_key, salt, b"aes256-derivation", Algorithm::AES256GCM).unwrap();
    assert_eq!(derived_aes256.secret_bytes().unwrap().as_bytes().len(), 32);
    assert!(derived_aes256.is_valid());
    
    // SM4 (16字节)
    let derived_sm4 = Hkdf::derive(&master_key, salt, b"sm4-derivation", Algorithm::SM4GCM).unwrap();
    assert_eq!(derived_sm4.secret_bytes().unwrap().as_bytes().len(), 16);
    assert!(derived_sm4.is_valid());
    
    // AES-128 (16字节)
    let derived_aes128 = Hkdf::derive(&master_key, salt, b"aes128-derivation", Algorithm::AES128GCM).unwrap();
    assert_eq!(derived_aes128.secret_bytes().unwrap().as_bytes().len(), 16);
    assert!(derived_aes128.is_valid());
    
    // AES-192 (24字节)
    let derived_aes192 = Hkdf::derive(&master_key, salt, b"aes192-derivation", Algorithm::AES192GCM).unwrap();
    assert_eq!(derived_aes192.secret_bytes().unwrap().as_bytes().len(), 24);
    assert!(derived_aes192.is_valid());
    
    // 验证不同算法派生的密钥是不同的
    assert_ne!(derived_aes256.secret_bytes().unwrap().as_bytes(), derived_aes128.secret_bytes().unwrap().as_bytes());
    assert_ne!(derived_aes256.secret_bytes().unwrap().as_bytes(), derived_aes192.secret_bytes().unwrap().as_bytes());
    assert_ne!(derived_aes128.secret_bytes().unwrap().as_bytes(), derived_sm4.secret_bytes().unwrap().as_bytes());
}
...
fn test_key_derivation_deterministic() {
    ...
    for _ in 0..5 {
        let derived = Hkdf::derive(&master_key, b"test_salt", b"test_context", Algorithm::AES256GCM).unwrap();
...
fn test_key_derivation_edge_cases() {
    ...
    // 测试空盐值
    let derived_empty_salt = Hkdf::derive(&master_key, b"", b"context", Algorithm::AES256GCM).unwrap();
    assert!(!derived_empty_salt.secret_bytes().unwrap().as_bytes().is_empty());
    
    // 测试空上下文
    let derived_empty_context = Hkdf::derive(&master_key, b"salt", b"", Algorithm::AES256GCM).unwrap();
    assert!(!derived_empty_context.secret_bytes().unwrap().as_bytes().is_empty());
    
    // 测试长上下文
    let long_context = vec![b'A'; 1000];
    let derived_long_context = Hkdf::derive(&master_key, b"salt", &long_context, Algorithm::AES256GCM).unwrap();
    assert!(!derived_long_context.secret_bytes().unwrap().as_bytes().is_empty());
...
fn test_key_derivation_security_properties() {
    ...
    assert!(difference_ratio > 0.75, "Key derivation should have strong avalanche effect");
}
