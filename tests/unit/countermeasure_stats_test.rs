...
use ciphern::side_channel::{SideChannelConfig, SideChannelContext};
...
fn test_countermeasure_stats_isolation() {
    ...
    let mut context2 = SideChannelContext::new(config2);
...
fn test_countermeasure_stats_with_arc_mutex() {
    ...
    // 等待两个线程完成
    let stats1 = handle1.join().unwrap();
    let stats2 = handle2.join().unwrap();
