...
fn run_latency_test(
    ...
    for &size in payload_sizes {
        ...
        for _ in 0..TEST_ITERATIONS {
            let latency = measure_decryption_latency(&cipher, &key_manager, &key_id, &encrypted_payload);
        ...
        results.insert(
            format!("encrypt_{}_bytes", size),
            encrypt_stats,
        );
        results.insert(
            format!("decrypt_{}_bytes", size),
            decrypt_stats,
        );
    }
...
fn test_api_communication_latency_aes256() {
    ...
    for (operation, stats) in &results {
        ...
        println!("\n{}:", operation);
        println!("  P99: {:.3}ms (threshold: {:.1}ms) - {}", 
                 p99_ms, P99_THRESHOLD_MS, 
                 if passed { "✅ PASS" } else { "❌ FAIL" });
        println!("  Min: {:.3}ms, Max: {:.3}ms, Mean: {:.3}ms",
                 stats.min.as_secs_f64() * 1000.0,
                 stats.max.as_secs_f64() * 1000.0,
                 stats.mean.as_secs_f64() * 1000.0);
        println!("  P50: {:.3}ms, P90: {:.3}ms, P999: {:.3}ms",
                 stats.p50.as_secs_f64() * 1000.0,
                 stats.p90.as_secs_f64() * 1000.0,
                 stats.p999.as_secs_f64() * 1000.0);
    }
    
    assert!(all_tests_passed, "Some latency tests failed P99 threshold of {}ms", P99_THRESHOLD_MS);
}
...
fn test_end_to_end_api_latency() {
    ...
    let request = b"{\"action\":\"transfer\",\"amount\":1000,\"from\":\"user123\",\"to\":\"user456\"}";
    ...
    for _ in 0..100 {
        ...
        let _decrypted_req = cipher.decrypt(&key_manager, &key_id, &encrypted_req).unwrap();
        let encrypted_res = cipher.encrypt(&key_manager, &key_id, response).unwrap();
        let _decrypted_res = cipher.decrypt(&key_manager, &key_id, &encrypted_res).unwrap();
    }
    ...
    for _ in 0..TEST_ITERATIONS {
        ...
        let decrypted_req = cipher.decrypt(&key_manager, &key_id, &encrypted_req).unwrap();
        ...
        let decrypted_res = cipher.decrypt(&key_manager, &key_id, &encrypted_res).unwrap();
    ...
    assert!(passed, "End-to-end API latency test failed P99 threshold of {}ms", P99_THRESHOLD_MS);
}