...
pub use lifecycle::{KeyLifecycleManager, KeyLifecyclePolicy, KeyVersion, KeyManagerLifecycleExt};
...
impl Key {
    ...
    pub fn destroy(&mut self) -> Result<()> {
        ...
        let zeroized_key = crate::memory::SecretBytes::new(vec![0u8; self.data.access()?.as_bytes().len()])?;
    ...
    pub fn increment_usage(&mut self) -> Result<()> {
        if !self.is_valid() {
            return Err(crate::error::CryptoError::KeyError("Key is not valid for use".into()));
        }
...
impl Drop for Key {
    fn drop(&mut self) {
        ...
        if let Ok(_secret) = self.data.access() {
            if let Ok(zeroized_key) = crate::memory::SecretBytes::new(vec![0u8; _secret.as_bytes().len()]) {
                self.data = crate::memory::ProtectedKey::new(zeroized_key);
            }
        }
    }
