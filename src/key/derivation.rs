...
impl Hkdf {
    pub fn derive(
        ...
        // 其他大小的密钥不支持
        Err(CryptoError::InvalidParameter(format!("Unsupported key size: {}", key_size)))
    }
    ...
    fn derive_32_bytes(
        ...
        let okm = prk.expand(&info_array, hkdf::HKDF_SHA256)
            .map_err(|e| {
                CryptoError::EncryptionFailed(format!("HKDF Expand failed: {:?}", e))
            })?;
        
        okm.fill(&mut derived_bytes)
            .map_err(|e| {
                CryptoError::EncryptionFailed(format!("HKDF Fill failed: {:?}", e))
            })?;
...
impl Pbkdf2 {
    pub fn derive(
        ...
        pbkdf2::<Hmac<Sha256>>(
            ...
            CryptoError::EncryptionFailed(format!("PBKDF2 failed: {:?}", e))
        })?;
...
impl Argon2id {
    pub fn derive(
        ...
        let params = Params::new(
            ...
            CryptoError::InvalidParameter(format!("Argon2 params error: {:?}", e))
        })?;
        ...
        argon2.hash_password_into(
            ...
            CryptoError::EncryptionFailed(format!("Argon2id failed: {:?}", e))
        })?;
