...
use super::{Key, KeyState, KeyLifecycleManager, KeyManagerLifecycleExt, KeyManagerOperations};
...
impl KeyManager {
    ...
    pub fn generate_key(&self, algorithm: Algorithm) -> Result<String> {
        ...
        {
            let mut store = self.keys.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
    ...
    pub fn generate_key_with_alias(&self, algorithm: Algorithm, alias: &str) -> Result<String> {
        ...
        // 设置别名映射
        let mut aliases = self.key_aliases.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
    ...
    pub fn resolve_alias(&self, alias: &str) -> Result<String> {
        let aliases = self.key_aliases.read().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
        aliases.get(alias).cloned().ok_or_else(|| CryptoError::KeyNotFound(alias.to_string()))
    }
    
    /// 获取密钥（支持别名）
    pub fn get_key(&self, id_or_alias: &str) -> Result<Key> {
        // 首先尝试作为别名解析
        let key_id = self.resolve_alias(id_or_alias).unwrap_or_else(|_| id_or_alias.to_string());
        
        let store = self.keys.read().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
        let key = store.get(&key_id).ok_or_else(|| CryptoError::KeyNotFound(key_id.clone()))?;
    ...
    pub fn activate_key(&self, id_or_alias: &str) -> Result<()> {
        ...
        let mut store = self.keys.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
        let key = store.get_mut(&key_id).ok_or_else(|| CryptoError::KeyNotFound(key_id.clone()))?;
    ...
    pub fn get_key_stats(&self) -> Result<HashMap<String, String>> {
        ...
        let active_keys = store.values().filter(|k| k.state() == KeyState::Active).count();
        let suspended_keys = store.values().filter(|k| k.state() == KeyState::Suspended).count();
        let destroyed_keys = store.values().filter(|k| k.state() == KeyState::Destroyed).count();
...
impl KeyManagerLifecycleExt for KeyManager {
    fn generate_key_with_lifecycle(&self, algorithm: Algorithm, lifecycle_manager: &KeyLifecycleManager) -> Result<String> {
        lifecycle_manager.create_key_version(self, algorithm)
    }
    
    fn rotate_key(&self, key_id: &str, algorithm: Algorithm, lifecycle_manager: &KeyLifecycleManager) -> Result<String> {
        lifecycle_manager.rotate_key(self, key_id, algorithm)
    }
    
    fn get_key_lifecycle_status(&self, key_id: &str, lifecycle_manager: &KeyLifecycleManager) -> Result<String> {
        lifecycle_manager.get_rotation_warning(key_id)
...
impl TenantKeyManager {
        ...
        if key_id.contains(':') {
            ...
            if parts[0] != self.tenant_id {
                ...
                AuditLogger::log_unauthorized_access(
                    ...
                    Some(key_id),
                    Some(&self.tenant_id),
                    &format!("Tenant {} attempted to access key from tenant {}", self.tenant_id, parts[0])
                );
                return Err(crate::error::CryptoError::KeyNotFound("Key not found in tenant".into()));
            }
            Ok(key_id.to_string())
        } else {
            // 添加租户前缀
    ...
    pub fn generate_key(&self, algorithm: Algorithm) -> Result<String> {
        ...
        AuditLogger::log_with_tenant("KEY_GENERATE", Some(algorithm), Some(&tenant_key_id), Some(&self.tenant_id), Ok(()), "authorized");
        ...
        {
            let mut store = self.key_manager.keys.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
    ...
    pub fn generate_key_with_alias(&self, algorithm: Algorithm, alias: &str) -> Result<String> {
        ...
        self.key_manager.generate_key_with_alias(algorithm, &tenant_alias)
    }
    ...
    pub fn list_keys(&self) -> Result<Vec<String>> {
        ...
        let tenant_keys: Vec<String> = all_keys
            .into_iter()
            .filter(|key_id| key_id.starts_with(&format!("{}:", self.tenant_id)))
            .map(|key_id| key_id.strip_prefix(&format!("{}:", self.tenant_id)).unwrap().to_string())
            .collect();
    ...
    pub fn set_key_max_usage(&self, key_id: &str, max_usage: Option<usize>) -> Result<()> {
        ...
        let result = self.key_manager.set_key_max_usage(&tenant_key_id, max_usage);
