...
impl KeyLifecycleManager {
    ...
    pub fn set_policy(&self, algorithm: Algorithm, policy: KeyLifecyclePolicy) -> Result<()> {
        let mut policies = self.policies.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
    ...
    /// 创建密钥版本
    pub fn create_key_version(&self, key_manager: &dyn KeyManagerOperations, algorithm: Algorithm) -> Result<String> {
        ...
        // 存储版本信息
        let mut versions = self.key_versions.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
        ...
        if policy.auto_rotation_enabled {
            let mut schedule = self.rotation_schedule.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
    ...
    pub fn get_active_version(&self, key_id: &str) -> Result<KeyVersion> {
        let versions = self.key_versions.read().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
        let key_versions = versions.get(key_id).ok_or_else(|| CryptoError::KeyNotFound(key_id.to_string()))?;
    ...
    /// 执行密钥轮换
    pub fn rotate_key(&self, key_manager: &dyn KeyManagerOperations, key_id: &str, algorithm: Algorithm) -> Result<String> {
        // 检查是否需要轮换
    ...
    pub fn increment_key_usage(&self, key_id: &str) -> Result<()> {
        let mut versions = self.key_versions.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
        let key_versions = versions.get_mut(key_id).ok_or_else(|| CryptoError::KeyNotFound(key_id.to_string()))?;
    ...
    pub fn get_rotation_warning_for_key(&self, key_manager: &dyn KeyManagerOperations, key_id: &str) -> Result<Option<String>> {
        // 首先检查密钥是否存在
    ...
    /// 销毁密钥及其所有版本
    pub fn destroy_key_with_versions(&self, key_manager: &dyn KeyManagerOperations, key_id: &str) -> Result<()> {
        // 销毁所有版本
        let mut versions = self.key_versions.write().map_err(|_| CryptoError::MemoryProtectionFailed("Lock poisoned".into()))?;
    ...
    /// 执行批量密钥轮换
    pub fn rotate_all_expired_keys(&self, key_manager: &dyn KeyManagerOperations) -> Result<Vec<String>> {
