...
impl CpuSignatureKernel {
    ...
    pub fn ed25519_verify(&self, public_key: &[u8], data: &[u8], signature: &[u8]) -> Result<bool> {
        ...
        let ed25519_signature = Ed25519Signature::from_slice(signature)
            .map_err(|e| CryptoError::InvalidInput(e.to_string()))?;

        let result = verifying_key
            .verify(data, &ed25519_signature)
            .map_err(|e| CryptoError::VerificationFailed(e.to_string()));
...
mod tests {
    ...
    fn test_ecdsa_sign_verify() {
        ...
        let public_key = [
            4u8, 121, 190, 102, 62, 80, 82, 196, 93, 219, 89, 20, 20, 58, 111, 44, 222, 160, 96,
            116, 147, 72, 65, 189, 244, 98, 117, 80, 86, 183, 42, 170, 158, 234, 195, 176, 244,
            169, 25, 103, 73, 23, 188, 178, 105, 87, 251, 149, 210, 67, 158, 200, 191, 238, 255,
            137, 218, 66, 82, 239, 167, 235, 210,
        ];
    ...
    fn test_ecdsa_verify_invalid_signature() {
        ...
        let result =
            kernel.ecdsa_verify(&public_key, data, &invalid_signature, Algorithm::ECDSA256);
        assert!(result.is_ok());
        assert!(!result.unwrap());
    }
    ...
    fn test_ecdsa_batch_verify() {
        ...
        for i in 0..5 {
            ...
            let signature = kernel
                .ecdsa_sign(&private_key, &data, Algorithm::ECDSA256)
                .unwrap();

            public_keys.push(&[
                4u8, 121, 190, 102, 62, 80, 82, 196, 93, 219, 89, 20, 20, 58, 111, 44, 222, 160,
                96, 116, 147, 72, 65, 189, 244, 98, 117, 80, 86, 183, 42, 170, 158, 234, 195, 176,
                244, 169, 25, 103, 73, 23, 188, 178, 105, 87, 251, 149, 210, 67, 158, 200, 191,
                238, 255, 137, 218, 66, 82, 239, 167, 235, 210,
            ]);
        ...
        let results =
            kernel.ecdsa_verify_batch(&public_keys, &data_items, &signatures, Algorithm::ECDSA256);
