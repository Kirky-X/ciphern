...
impl BooleanMasking {
    ...
    /// Mask a byte array bit by bit
    pub fn mask_bytes(&self, values: &[u8]) -> Vec<u8> {
        let mut result = values.to_vec();
        
        for (i, byte) in result.iter_mut().enumerate() {
            for bit in 0..8 {
                if i * 8 + bit < self.masks.len() {
                    let bit_mask = self.masks[i * 8 + bit];
                    let bit_value = (*byte >> bit) & 1;
                    let masked_bit = bit_value ^ (bit_mask as u8);
                    *byte = (*byte & !(1 << bit)) | (masked_bit << bit);
                }
            }
        }
...
impl HigherOrderMasking {
    ...
    pub fn unmask(&self) -> Result<Vec<u8>> {
        if self.shares.len() != self.order + 1 {
            return Err(CryptoError::InvalidParameter("Invalid number of shares".into()));
        }
    ...
    pub fn refresh(&mut self) -> Result<()> {
        ...
        // Generate new random masks for first n-1 shares
        for i in 0..self.order {
            let mut new_mask = vec![0u8; self.shares[i].len()];
            SecureRandom::new()?.fill(&mut new_mask)?;
            
            // Apply mask to share i and unmask from share n
            for j in 0..self.shares[i].len() {
                self.shares[i][j] ^= new_mask[j];
                self.shares[self.order][j] ^= new_mask[j];
            }
        }
...
impl MaskedLookupTable {
    ...
    pub fn lookup(&self, masked_input: u8) -> u8 {
        ...
        // Unmask the value
        let result = masked_value ^ self.output_mask;
        
        println!("  lookup: masked_input=0x{:02x}, table_index={}, masked_value=0x{:02x}, result=0x{:02x}", 
                 masked_input, table_index, masked_value, result);
...
mod tests {
    ...
    fn test_masked_lookup_table() {
        ...
        for (i, &expected) in table.iter().enumerate() {
            ...
            println!("Index {}: masked_input=0x{:02x}, expected=0x{:02x}, result=0x{:02x}", 
        ...
        for &invalid_idx in &invalid_indices {
            ...
            let result = masked_table.lookup(masked_input);
            println!("Invalid index {}: masked_input=0x{:02x}, result=0x{:02x}", 
