...
impl SideChannelProtectionTester {
    ...
    pub fn test_aes128_timing_protection(&self) -> TimingAttackTestResult {
        ...
        config.timing_noise_enabled = true;
    ...
    pub fn test_aes192_power_analysis_protection(&self) -> PowerAnalysisTestResult {
        let mut config = SideChannelConfig::default();
        config.power_analysis_protection = true;
        config.masking_operations_enabled = true;
    ...
    pub fn test_aes256_error_injection_protection(&self) -> ErrorInjectionTestResult {
        ...
        config.redundancy_checks_enabled = true;
        ...
        let false_positive_rate = false_positive_count as f64 / (self.config.iterations * 9 / 10) as f64;
    ...
    pub fn test_sm4_cache_attack_protection(&self) -> CacheAttackTestResult {
        ...
        config.cache_flush_enabled = true;
    ...
    fn analyze_timing_patterns(&self, measurements: &[Duration]) -> TimingAttackTestResult {
        ...
        let filtered_times: Vec<f64> = times_ns.iter()
        ...
        if outlier_percentage > 20.0 {
            println!("警告: 异常值比例过高 ({:.1}%)，测量环境可能不稳定", outlier_percentage);
        }
    ...
    fn calculate_basic_statistics(&self, times_ns: &[f64], outlier_count: usize, outlier_percentage: f64) -> TimingAttackTestResult {
        let average = times_ns.iter().sum::<f64>() / times_ns.len() as f64;
        let variance = times_ns.iter().map(|x| (x - average).powi(2)).sum::<f64>() / times_ns.len() as f64;
        let std_deviation = variance.sqrt();
        
        let coefficient_of_variation = if average > 0.0 { std_deviation / average } else { 0.0 };
        
        // 基本统计：由于异常值过多，使用更宽松的阈值
        let timing_leakage = coefficient_of_variation > 2.0; // 200% 变异系数
        
        let max_deviation = times_ns.iter()
            .map(|x| (x - average).abs())
            .fold(0.0f64, f64::max);
        
        println!("时序分析详细统计 (基本统计):");
        println!("  样本数量: {} (异常值: {} 个, {:.1}%)", times_ns.len(), outlier_count, outlier_percentage);
        println!("  平均值: {:.2} ns", average);
        println!("  标准差: {:.2} ns", std_deviation);
        println!("  变异系数: {:.3} ({}%)", coefficient_of_variation, coefficient_of_variation * 100.0);
    ...
    fn calculate_robust_statistics(&self, filtered_times: &[f64], median: f64, outlier_count: usize, outlier_percentage: f64) -> TimingAttackTestResult {
        let robust_average = filtered_times.iter().sum::<f64>() / filtered_times.len() as f64;
        let robust_variance = filtered_times.iter().map(|x| (x - robust_average).powi(2)).sum::<f64>() / filtered_times.len() as f64;
        ...
        let max_deviation_filtered = filtered_times.iter()
            .map(|x| (x - robust_average).abs())
            .fold(0.0f64, f64::max);
        
        println!("时序分析详细统计 (稳健统计):");
        println!("  原始样本数量: {} (异常值: {} 个, {:.1}%)", 
                 filtered_times.len() + outlier_count, outlier_count, outlier_percentage);
        ...
        println!("  稳健变异系数: {:.3} ({}%)", robust_coefficient_of_variation, robust_coefficient_of_variation * 100.0);
    ...
    fn analyze_power_patterns(&self, measurements: &[f64], masking_count: usize) -> PowerAnalysisTestResult {
    ...
    fn analyze_cache_patterns(&self, access_patterns: &[Duration], timing_variations: &[Duration]) -> CacheAttackTestResult {
        // 分析缓存访问模式的规律性
    ...
    fn calculate_power_correlation(&self, measurements: &[f64]) -> f64 {
        ...
        let sum_xy: f64 = measurements.iter().enumerate().map(|(i, &x)| x * i as f64).sum();
    ...
    fn simulate_power_measurement(&self, provider: &Aes192GcmProvider, key: &Key, plaintext: &[u8]) -> f64 {
    ...
    fn simulate_error_injection(&self, provider: &Aes256GcmProvider, key: &Key, plaintext: &[u8], should_inject: bool) -> Result<Vec<u8>> {
        if should_inject {
            // 模拟错误注入
            Err(crate::error::CryptoError::DecryptionFailed("Simulated error injection".into()))
        } else {
...
pub fn run_strict_side_channel_tests() -> ComprehensiveTestResult {
    let mut config = SideChannelTestConfig::default();
    config.iterations = 5000;
    config.timing_threshold_ns = 500; // 更严格的时序要求
    config.power_analysis_threshold = 0.05; // 更严格的功耗分析要求
    config.error_detection_threshold = 0.98; // 更高的错误检测要求
...
mod tests {
    ...
    fn test_standard_side_channel_protection() {
        ...
        println!("  最大偏差: {:.2} ns", result.aes128_timing.max_deviation_ns);
        ...
        // 验证基本防护功能
        assert!(result.aes128_timing.protection_effective, "AES128时序防护应该有效");
        assert!(result.aes192_power.protection_effective, "AES192功耗分析防护应该有效");
        assert!(result.aes256_error.protection_effective, "AES256错误注入防护应该有效");
        assert!(result.sm4_cache.protection_effective, "SM4缓存攻击防护应该有效");
    }
