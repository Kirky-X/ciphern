    ...
    fn randomize_cache_access(&self) -> Result<()> {
        ...
        THREAD_RNG.with(|rng| {
            ...
            let byte_slice = unsafe {
                std::slice::from_raw_parts_mut(indices.as_mut_ptr() as *mut u8, 
        ...
        for i in 0..indices.len() {
            indices[i] = indices[i] % self.config.dummy_access_count;
        }
...
impl CacheTimingMeasurer {
    ...
    pub fn detect_anomaly(&self) -> Result<bool> {
        ...
        let variance_sum: f64 = self.measurements
            .iter()
            .map(|&d| {
                let diff = if d > average {
                    d - average
                } else {
                    average - d
                };
        ...
        for &measurement in &self.measurements {
            let deviation = if measurement > average {
                measurement - average
            } else {
                average - measurement
            };
...
mod tests {
    ...
    fn test_cache_partition() {
        ...
        // Debug output
        println!("Touched count: {}, Expected count: {}", touched_count, expected_count);
    ...
    fn test_cache_protection_guard() {
        ...
        let result = guard.protect_access(|| {
            Ok::<_, CryptoError>(42)
        });
