...
pub extern "C" fn ciphern_generate_key(
    ...
    match std::panic::catch_unwind(|| {
        with_context(|context| {
            // 验证参数
            let algo_str = unsafe { validation::validate_c_str(algorithm_name) }.map_err(|_e| CiphernError::InvalidParameter)?;
            // output buffer should not be validated as c_str (which implies reading it)
            validation::validate_mut_ptr(key_id_buffer, "key_id_buffer").map_err(|_e| CiphernError::InvalidParameter)?;
...
pub extern "C" fn ciphern_encrypt(
    ...
    match std::panic::catch_unwind(|| {
        with_context(|context| {
            // 验证参数
            let key_id_str = unsafe { validation::validate_c_str(key_id) }.map_err(|_e| CiphernError::InvalidParameter)?;
            let plaintext_slice = unsafe { validation::validate_slice(plaintext, plaintext_len) }.map_err(|_e| CiphernError::InvalidParameter)?;
            let ciphertext_buffer = unsafe { validation::validate_mut_slice(ciphertext, ciphertext_buffer_size) }.map_err(|_e| CiphernError::InvalidParameter)?;
            let ciphertext_len_ptr = unsafe { validation::validate_mut_usize(ciphertext_len, "ciphertext_len") }.map_err(|_e| CiphernError::InvalidParameter)?;
