...
impl RsaProvider {
    ...
    fn get_signing_alg(&self) -> Result<&'static dyn ring::signature::RsaEncoding> {
        match self.algorithm {
            ...
            _ => Err(CryptoError::UnsupportedAlgorithm(format!("{:?}", self.algorithm))),
        }
    }
}
...
impl Signer for RsaProvider {
    fn sign(&self, key: &Key, message: &[u8]) -> Result<Vec<u8>> {
        if key.algorithm() != self.algorithm {
            return Err(CryptoError::UnsupportedAlgorithm("Key algorithm mismatch".into()));
        }
        ...
        key_pair.sign(self.get_signing_alg()?, &rng, message, &mut signature)
    ...
    fn verify(&self, key: &Key, message: &[u8], signature: &[u8]) -> Result<bool> {
        ...
        let verification_alg = match self.algorithm {
            Algorithm::RSA2048 | Algorithm::RSA3072 | Algorithm::RSA4096 => &ring::signature::RSA_PKCS1_2048_8192_SHA256,
            _ => return Err(CryptoError::UnsupportedAlgorithm(format!("{:?}", self.algorithm))),
        };

        let public_key = ring::signature::UnparsedPublicKey::new(verification_alg, public_key_bytes);
