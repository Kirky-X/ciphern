...
impl FipsContext {
    ...
    pub fn validate_algorithm(&self, algorithm: &Algorithm) -> Result<()> {
        ...
        if let Err(e) = self.self_test_engine.run_conditional_self_test(*algorithm) {
            self.error_state.set_error(FipsError::ConditionalSelfTestFailed(
                format!("{:?}", algorithm),
                e.to_string()
            ));
            return Err(e);
    ...
    pub fn run_periodic_self_tests(&self) -> Result<()> {
        ...
        if !rng_test.passed {
            self.error_state.set_error(FipsError::RngHealthCheckFailed(
                rng_test.error_message.unwrap_or_else(|| "Unknown RNG error".to_string())
            ));
            return Err(CryptoError::FipsError("Periodic self test failed".to_string()));
        }
