    ...
    pub fn validate_key_size(algorithm: &Algorithm, key_size: usize) -> Result<()> {
        match algorithm {
            ...
            Algorithm::AES128GCM if key_size != 16 => {
                return Err(CryptoError::FipsError(format!(
                    "AES-128 requires 16 byte key, got {}", key_size
                )));
            },
            Algorithm::AES192GCM if key_size != 24 => {
                return Err(CryptoError::FipsError(format!(
                    "AES-192 requires 24 byte key, got {}", key_size
                )));
            },
            Algorithm::AES256GCM if key_size != 32 => {
                return Err(CryptoError::FipsError(format!(
                    "AES-256 requires 32 byte key, got {}", key_size
                )));
            },
            
            // RSA 密钥大小要求 (FIPS 140-3 要求最小 2048 位)
            Algorithm::RSA2048 if key_size < 256 => { // 2048 bits = 256 bytes
                return Err(CryptoError::FipsError(format!(
                    "RSA-2048 requires at least 256 byte key, got {}", key_size
                )));
            },
            Algorithm::RSA3072 if key_size < 384 => { // 3072 bits = 384 bytes
                return Err(CryptoError::FipsError(format!(
                    "RSA-3072 requires at least 384 byte key, got {}", key_size
                )));
            },
            Algorithm::RSA4096 if key_size < 512 => { // 4096 bits = 512 bytes
                return Err(CryptoError::FipsError(format!(
                    "RSA-4096 requires at least 512 byte key, got {}", key_size
                )));
            },
            ...
            Algorithm::ECDSAP256 if key_size < 32 => {
                return Err(CryptoError::FipsError(format!(
                    "ECDSA P-256 requires at least 32 byte key, got {}", key_size
                )));
            },
            Algorithm::ECDSAP384 if key_size < 48 => {
                return Err(CryptoError::FipsError(format!(
                    "ECDSA P-384 requires at least 48 byte key, got {}", key_size
                )));
            },
            Algorithm::ECDSAP521 if key_size < 66 => {
                return Err(CryptoError::FipsError(format!(
                    "ECDSA P-521 requires at least 66 byte key, got {}", key_size
                )));
            },
