...
pub fn record_performance_operation(
    ...
    PERFORMANCE_MONITOR.record_operation(operation_id, operation_type, algorithm, data_size_bytes, cache_key)
}
...
impl PerformanceMonitor {
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        let history_size = self.metrics_history.read().unwrap().len() * std::mem::size_of::<PerformanceMetrics>();
    ...
    fn update_stats(&self, metrics: &PerformanceMetrics) {
        ...
        if stats.total_operations == 1 {
            ...
            stats.avg_latency_us = old_avg + (metrics.latency_us as f64 - old_avg) / stats.total_operations as f64;
        ...
        if stats.total_operations == 1 {
            ...
            stats.avg_throughput_ops_per_sec = old_avg + (metrics.throughput_ops_per_sec - old_avg) / stats.total_operations as f64;
        }
        ...
        if stats.total_operations == 1 {
            ...
            stats.avg_cache_hit_rate = old_avg + (metrics.cache_hit_rate - old_avg) / stats.total_operations as f64;
        }
    ...
    fn calculate_performance_trend(&self) -> String {
        ...
        let recent_avg_latency: f64 = recent_metrics.iter().map(|m| m.latency_us as f64).sum::<f64>() / recent_count as f64;
        let older_avg_latency: f64 = older_metrics.iter().map(|m| m.latency_us as f64).sum::<f64>() / recent_count as f64;
...
impl AuditLogger {
    ...
    pub fn log(operation: &str, algo: Option<Algorithm>, key_id: Option<&str>, result: Result<(), &str>) {
    ...
    /// 记录带有租户ID的审计日志
    pub fn log_with_tenant(operation: &str, algo: Option<Algorithm>, key_id: Option<&str>, tenant_id: Option<&str>, result: Result<(), &str>, access_type: &str) {
        let status = match result {
            Ok(_) => "SUCCESS",
            Err(_) => "FAILURE",
        };
    ...
    /// 记录非授权访问尝试
    pub fn log_unauthorized_access(operation: &str, algo: Option<Algorithm>, key_id: Option<&str>, tenant_id: Option<&str>, details: &str) {
        let entry = AuditLog {
            timestamp: Utc::now(),
            operation: operation.to_string(),
            algorithm: algo,
