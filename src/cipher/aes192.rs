...
use aes_gcm::{AesGcm, aes::Aes192, AeadCore, KeyInit};
...
impl Aes192GcmProvider {
    ...
    pub fn with_side_channel_config(config: SideChannelConfig) -> Self {
        let rotating_sbox = if config.power_analysis_protection {
            RotatingSboxMasking::new(4).ok().map(|sbox| Arc::new(Mutex::new(sbox))) // 4 rotating S-boxes if power analysis protection enabled
        } else {
            None
        };
    ...
    fn encrypt_core(&self, key: &Key, plaintext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>> {
        ...
        // Apply side-channel protected key expansion if available
        let protected_key = if self.rotating_sbox.is_some() {
            self.expand_key_protected(secret.as_bytes())?
        } else {
            secret.as_bytes().to_vec()
        };
        
        // Note: We're using AES-128Gcm which can handle different key sizes including 192-bit keys
        let cipher = AesGcm::<Aes192, U12>::new_from_slice(&protected_key)
            .map_err(|_| CryptoError::EncryptionFailed("Invalid Key".into()))?;

        let nonce = AesGcm::<Aes192, U12>::generate_nonce(&mut SecureRandom::new()?);
        
        let ciphertext = if let Some(_aad_data) = aad {
    ...
    fn decrypt_core(&self, key: &Key, ciphertext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>> {
        ...
        if ciphertext.len() < 12 { // Nonce size
        ...
        let plaintext = if let Some(_aad_data) = aad {
    ...
    fn expand_key_protected(&self, key_bytes: &[u8]) -> Result<Vec<u8>> {
        ...
        if let Some(ref sbox_masking) = self.rotating_sbox {
            ...
            let mut expanded_key = Vec::with_capacity(208); // AES-192 expanded key size
                ...
                if i > 5 && i % 4 == 0 {
                    ...
                    for j in 0..4 {
                        if byte_idx + j < expanded_key.len() {
                            ...
                            let mut sbox = sbox_masking.lock().unwrap();
...
mod tests {
    ...
    fn test_aes192_with_side_channel_protection() {
        ...
        assert!(stats.timing_protections > 0 || stats.masking_operations > 0 || 
                stats.error_detection_triggers > 0 || stats.cache_flush_operations > 0);
    }
...
impl SymmetricCipher for Aes192GcmProvider {
    ...
    fn decrypt(&self, key: &Key, ciphertext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>> {
        if ciphertext.len() < 12 + 16 { // Nonce + Tag min
            return Err(CryptoError::DecryptionFailed("Ciphertext too short".into()));
        }
...
impl Aes192GcmProvider {
    ...
    pub fn get_side_channel_stats(&self) -> Option<crate::side_channel::SideChannelStats> {
        self.side_channel_context.as_ref().map(|ctx| ctx.lock().unwrap().get_stats())
    }
