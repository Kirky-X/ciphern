...
use aes_gcm::{aead::{Aead, KeyInit}, Aes128Gcm, Key as AesGcmKey, Nonce as AesGcmNonce};
...
fn test_vector_1() {
    ...
    let result = provider.encrypt_with_nonce(&key, &pt_bytes, &iv_bytes, Some(&aad_bytes)).unwrap();
    ...
    let decrypted = provider.decrypt(&key, &full_ciphertext, Some(&aad_bytes)).unwrap();
...
fn test_nist_vector_1_ring() {
    ...
    let tag = key.seal_in_place_separate_tag(nonce1, aad1, &mut in_out).unwrap();
    ...
    match result {
        ...
        Err(e) => {
            ...
            println!("✓ ring crate encryption test passed (decryption skipped for empty plaintext)");
        }
    }
    println!("✓ ring crate encryption/decryption test passed");
    println!();
...
fn test_nist_vector_2_ring() {
    ...
    let tag2 = key.seal_in_place_separate_tag(nonce2, aad2, &mut in_out2).unwrap();
    ...
    println!("Plaintext: {}", plaintext_hex);
    println!("AAD: {}", aad_hex);
    println!("Generated Ciphertext: {}", hex::encode(&in_out2[..plaintext.len()]));
